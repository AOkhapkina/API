...
//Почему бы не SpringApplication.run(ReconTool.class, args)?
// Потому что таким образом пружина настроена не полностью (без сканирования компонентов и т. д.). Создается только bean-компонент,
// определенный в run () (ReconTool).
